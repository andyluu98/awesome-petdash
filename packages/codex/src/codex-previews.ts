import type { CodexMcpEntry, CodexMcpPreviewOptions } from "./codex-mcp.js";
import { buildCodexMcpEntry } from "./codex-mcp.js";

export interface CodexRedactedPreview {
  readonly openpets?: CodexMcpEntry;
...
export function buildCodexOnlyPreview(options: CodexMcpPreviewOptions): CodexRedactedPreview {
  return { openpets: buildCodexMcpEntry(options) };
}

export function redactCodexConfig(config: Record<string, unknown>): Record<string, unknown> {
  const result: Record<string, unknown> = {};

  for (const [key, value] of Object.entries(config)) {
    if (key === "mcp_servers" && isRecord(value)) {
